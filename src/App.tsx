...
function makeVoicePlan(topic:string,count:number){
  ...
  if(fireworks){
    const beats=[
      {title:"도착",description:"해 질 무렵 두 친구가 여의도 한강공원에 도착해 한강이 잘 보이는 자리를 찾는다.",voiceType:"대사" as VoiceType,voiceText:"저쪽이 잘 보이겠다. 거기로 가자.",caption:"불꽃을 기다리는 저녁"},
      ...
      {title:"첫 불꽃",description:"한강 위로 첫 불꽃이 터지고 두 친구가 동시에 고개를 들어 밤하늘을 바라본다.",voiceType:"대사" as VoiceType,voiceText:"와, 진짜 예쁘다.",caption:"드디어 시작"},
      {title:"몰입",description:"불꽃이 연이어 터지는 동안 두 친구는 휴대폰을 잠시 내리고 직접 밤하늘을 바라본다.",voiceType:"없음" as VoiceType,voiceText:"",caption:"눈으로 담는 순간"},
      ...
      {title:"마무리",description:"마지막 불꽃이 사라진 뒤 두 친구가 돗자리를 접고 사람들과 함께 천천히 걸어 나간다.",voiceType:"대사" as VoiceType,voiceText:"오늘 오길 잘했다.",caption:"함께라서 더 오래 남는 밤"},
    ];
...
function makeScenes(topic:string,count:number,ratio:string,duration:string):Scene[]{
  ...
  return beats.map((beat,i)=>{
    ...
    const voiceInstruction=beat.voiceType==="없음"?"AUDIO: No spoken dialogue or narration. Use natural ambient sound only.":beat.voiceType==="대사"?`DIALOGUE: Speak this Korean line once, naturally and briefly: \"${beat.voiceText}\"`: `NARRATION: Use this Korean narration once in a calm, natural tone: \"${beat.voiceText}\"`;
