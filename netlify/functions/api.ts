...
const secret=()=>process.env.JWT_SECRET||"cinemag-change-this-secret";
const sign=(value:string)=>`${value}.${createHmac("sha256",secret()).update(value).digest("hex")}`;
const valid=(token:string)=>{const i=token.lastIndexOf(".");if(i<1)return false;const v=token.slice(0,i),sig=token.slice(i+1),expected=createHmac("sha256",secret()).update(v).digest("hex");try{return timingSafeEqual(Buffer.from(sig),Buffer.from(expected))}catch{return false}};
...
export default async (req:Request)=>{
 ...
 try{
  ...
  if(path==="login"&&req.method==="POST"){const input=await body(req) as any;const id=process.env.CINEMAG_ADMIN_ID||"admin";const pw=process.env.CINEMAG_ADMIN_PASSWORD||"change-this-password";if(input.id!==id||input.password!==pw)return json({error:"관리자 ID 또는 비밀번호가 올바르지 않습니다."},401);return json({token:sign(`admin:${Date.now()}`)});}
